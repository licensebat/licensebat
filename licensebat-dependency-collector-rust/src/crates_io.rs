...
use licensebat_core::{
    dependency_collector::RetrievedDependencyStreamResult, DependencyCollector,
...
#[derive(Debug)]
pub struct Rust<R: DependencyRetriever> {
    crates_io_retriever: Arc<R>,
}

// TODO: implement default by creating instances of the default retrievers
// impl<R: DependencyRetriever> Default for Rust<R> {
//     fn default() -> Self {
//         Self { crates_io_retriever: None }
//     }
// }

impl<R: DependencyRetriever> Rust<R> {
...
impl<R: DependencyRetriever> DependencyCollector for Rust<R> {
...
async fn get_dependency<R: DependencyRetriever>(
    package: Package,
    retriever: Arc<R>,
) -> RetrievedDependency {
...
mod tests {
    ...
    use licensebat_core::dependency_retriever::EmptyRetriever;

    #[tokio::test]
    async fn it_works_for_crates_registry() {
        let rust = Rust::new(EmptyRetriever::default());
        let lock_content = r#"
        [[package]]
        name = "mime"
        version = "0.3.16"
